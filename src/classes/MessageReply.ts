...
export class MessageReply {
  ...
  public async ShowConfig(): Promise<void> {
    ...
    Object.keys(descriptionObject).forEach((keyName) => {
      ...
      valuesArray.push(
        this.botGuild.GetConfig()[keyName] ? this.botGuild.GetConfig()[keyName] : this.botGuild.defaultConfig[keyName]
      );
  ...
  public async ShowSetConfig(): Promise<void> {
    ...
    if (key === undefined) {
      await this.message.channel.send(`Missing or unknown setting.`);
      return;
    }

    if (newValue === undefined) {
      await this.message.channel.send(`Missing value for \`${key}\``);
      return;
    }
  ...
  public async ShowSearch(): Promise<void> {
    const searchItem = this.rawParams.splice(1).join(" ").toLocaleLowerCase();
    const items = [];
  ...
  public ShowUserDKP(): void {
    ...
    if (items.length <= 0) {
      this.message.channel.send(
        `Sorry, I don't know player '${player}'.\nTry using *${trigger} search ${player}* if you are searching for an item.`
      );
      return;
    }
  ...
  public ShowDKPHelp(isAdmin: boolean, isDm: boolean): void {
    ...
    if (!isDm) {
      embed.addField("Start a direct message session with the bot", `${trigger} dm`);
    }
